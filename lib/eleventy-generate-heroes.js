...
function buildImageList (results, output) {
  ...
  results.forEach((result) => {
    if (result.outputPath.endsWith('/index.html')) {
      ...
      // hero image
      images.push({ url: result.url, folder, path: folder + 'hero.png', w: 2400, h: 800, transparent: true, });
      // og image
      images.push({ url: result.url, folder, path: folder + 'og.png', w: 1200, h: 630, transparent: false, });
    }
  });
  return images;
}
